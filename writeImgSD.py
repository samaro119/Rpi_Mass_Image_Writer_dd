...
def refreshLcd():
    ...
    if nowWriting:
        driveMessage = str(writeStatusLine) + "% complete"
    else :
        if justCompleted :
...
def runCommandAndGetStdout(cmd):
    ...
    return out.decode()  # convert bytes to string
...
def constructCommand():
    ...
    firstDriveCommand = " | sudo dd of=/dev/" + firstDriveName + " bs=1M"


    for i in range(1, numDrives):
        nextDriveCommand = " | tee >(sudo dd of=/dev/" + listOfDrives[i] + " bs=1M)"
